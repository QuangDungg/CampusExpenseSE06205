...
public class UserDb extends SQLiteOpenHelper {
    ...
    public boolean checkUsernameEmail(String username, String email){
        // kiem tra username va email co ton tai trong database khong?
        boolean checking = false;
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            String[] cols = {ID_COL, USERNAME_COL, EMAIL_COL};
            String condition = USERNAME_COL+ "=? AND " + EMAIL_COL + "=? ";
            String [] param = {username, email};
            Cursor cursor = db.query(TABLE_NAME, cols, condition, param, null, null, null);
            if (cursor.getCount() > 0 ){
                checking = true;
            }
            cursor.close();
